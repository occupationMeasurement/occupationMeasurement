...
Page <- R6::R6Class("Page",
  public = list(
    #' @field page_id A unique string identifiying this page. Used to store data.
    page_id = NULL,
    #' @field condition Function to check whether the page should be shown.
    condition = NULL,
    #' @field run_before Function that prepares data to render the page.
    run_before = NULL,
    #' @field render_before Called exactly like `render`. Output will be added just *before* the output from render.
    render_before = NULL,
    #' @field render Function to render the page. (Required)
    render = NULL,
    #' @field render_after Called exactly like `render`. Output will be added just *after* the output from render.
    render_after = NULL,
    #' @field run_after Function that handles the user input when they leave the page.
    run_after = NULL,
    ...
    #'   })
    #' )
    initialize = function(page_id, render, condition = NULL, run_before = NULL, render_before = NULL, render_after = NULL, run_after = NULL) {
      self$page_id <- page_id
      self$condition <- condition
      self$run_before <- run_before
      self$render_before <- render_before
      self$render <- render
    ...
    #' @keywords internal
    check_condition = function(session, ...) {
      if (!is.null(self$condition)) {
        return(self$condition(session = session, page = self, ...))
      } else {
        return(TRUE)
      }
    },
    ...
    #' @param ... All additional arguments are passed along
    #' @keywords internal
    execute_run_before = function(session, ...) {
      if (session$userData$app_settings$verbose) {
        cat("Page:", self$page_id, "\n")
      }

      # Initialize trial data e.g. set first timestamp, set ids etc.
      private$data_initialize(session)
      if (!is.null(self$run_before)) {
        return(self$run_before(session = session, page = self, ...))
      }
    },
    #' @description
    #' Called internally by the shiny server.
    ...
    #' @param ... All additional arguments are passed along
    #' @keywords internal
    execute_render = function(session, run_before_output, ...) {
      return(
        list(
          if (!is.null(self$render_before)) self$render_before(session = session, page = self, run_before_output = run_before_output, ...),
          self$render(session = session, page = self, run_before_output = run_before_output, ...),
          if (!is.null(self$render_after)) self$render_after(session = session, page = self, run_before_output = run_before_output, ...)
        )
      )
    },
    #' @description
    ...
    #' @param ... All additional arguments are passed along
    #' @keywords internal
    execute_run_after = function(session, input, ...) {
      # Finalize the trial data e.g. set the final timestamp
      private$data_finalize(session = session)

      if (!is.null(self$run_after)) {
        self$run_after(session = session, page = self, input = input, ...)
      }

      ## Save the page's data
      private$save_page_data(session = session)

      # Show a detailed message of the page's data if enabled
      if (session$userData$app_settings$verbose) {
        cat(
          "Page data for", self$page_id, ":",
          paste(capture.output(str(self$get_page_data(session = session))), collapse = "\n"),
          "\n"
        )
      }
    },
    ...
    #' ))
    #' }
    set_page_data = function(session, values) {
      # Add / Overwrite all provided values in the questionnaire data
      session$userData$questionnaire_data[[self$page_id]] <- utils::modifyList(
        session$userData$questionnaire_data[[self$page_id]],
        values
      )
    },
    #' @description
    ...
    #' }
    get_page_data = function(session, key = NULL, default = NULL, page_id = self$page_id) {
    ...
    #' )
    #' }
    set_question_data = function(session, page_id = self$page_id, question_id = NULL, question_text = NULL, response_text = NULL, response_id = NULL) {
    ...
    #' )
    #' }
    get_question_data = function(session, page_id = self$page_id, question_id = NULL, key = NULL, default = NULL) {
      if (is.null(question_id)) {
        question_id <- "default"
      }
      questions <- self$get_page_data(
  ...
  private = list(
    data_initialize = function(session) {
      if (is.null(session$userData$questionnaire_data[[self$page_id]])) {
        # Initialize page info
        session$userData$questionnaire_data[[self$page_id]] <- list(
    ...
    data_finalize = function(session) {
      session$userData$questionnaire_data[[self$page_id]]$end <- as.character(Sys.time())
      session$userData$questionnaire_data[[self$page_id]]$status <- "new"
    },
    save_page_data = function(session) {
      save_data(
        "answers",
        extract_questions_df(
          page_data = self$get_page_data(session = session)
        ),
        session = session
      )
    }
