...
![A diagram showing the participant flow within the interactive app](../man/figures/app_flow.svg)
...
- **session_id**: A unique session id, identifying this exaxct session / completion of the survey. (Format: `user_id`_`time stamp`_`random digit`)
...
- **user_id**: An external user_id to match data with another source of information. Only present if it is passed via the URL.
- **P_{page_id}_Q_{item_id}_R_{id/text}**: *Two* columns for every question answered by the respondent. These columns correspond to the naming convention ***P_{page_id}_Q_{item_id}_R_id*** for response ids and ***P_{page_id}_Q_{item_id}_R_text*** for response texts (see the documentation for answers for additional information).
...
Information on all sessions, saved whenever a session is ended (after a timeout, upon leaving the app or when stopping the server).
