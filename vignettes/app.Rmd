...
https://{url-to-my-interactive-app}/?user_id=<my_unique_interview_id>&tense=past&extra_instructions=off&num_suggestions=10
``` 
...
If you want to match your data with an external source or embed the interactive app within another survey, you will probably want to pass a unique interview or user identifier, a `id`, also called `user_id`.

This can be done adding a query parameter named `id` to the URL, i.e., `https://{url-to-my-interactive-app}/?user_id=my_unique_interview_id` (with {url-to-my-interactive-app} corresponding to the URL your application is running at and my_unique_interview_id corresponding to the unique id).
...
- **session_id**: A unique session id, identifying this exact session / completion of the survey. (Format: `user_id_Time stamp_Random digit)
...
A combined version of the user's answers and information about the user themselves. These are saved at the end of the questionnaire and therefore only once per participation. `session_id` is the unique key here. Note that `user_id` is not necessarily unique (although it should be) because the same `user_id` can be passed multiple times via the URL.

#### Columns

- **session_id**: A unique session id, identifying this exaxct session / completion of the survey. (Format: `user_id_Time stamp_Random digit)
