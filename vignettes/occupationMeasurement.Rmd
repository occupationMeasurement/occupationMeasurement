...
You're of course also not limited to using the app only with its default questionnaire. Indeed you can use any one of the ready-built questionnaires that come with the app, such as `questionnaire_web_survey()` (the default),`questionnaire_demo()` (to learn about the workings of the tool) or `questionnaire_interviewer_administerd()` (for guided interviews with a professional interviewer).
...
Moreover, you're also able to modify questionnaires or just build your own from sratch. You can read more about this functionality in `vignette("app-questionnaire")`.
...
4. If the suggested category has followup questions, retrieve the next followup question from `/v1/next_followup_question` and present it to the user.
